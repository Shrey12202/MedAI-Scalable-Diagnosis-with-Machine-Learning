...
# -----------------------
# Merge Simulation Data from MinIO with TRAIN_CSV
# -----------------------
# Local save dir for simulation images
SIM_IMAGE_DIR = os.path.join(IMG_ROOT, "patient-sim")
os.makedirs(SIM_IMAGE_DIR, exist_ok=True)

# MinIO config
MINIO_BUCKET = "production"
MINIO_IMG_PREFIX = "images/"
MINIO_JSON_PREFIX = "predictions/"
...
response = s3.list_objects_v2(Bucket=MINIO_BUCKET, Prefix=MINIO_JSON_PREFIX)
sim_records = []

if response.get("Contents"):
    print("📥 Simulation predictions found in MinIO...")
    ...
    for obj in response["Contents"]:
        ...
        # Convert confidences to binary labels
        label_vals = [int(data["predictions"].get(label, 0.0) >= 0.5) for label in LABELS]

        # Download corresponding image
        image_key = data["filename"]  # e.g., images/xxxx.jpg
        image_name = os.path.basename(image_key).replace(".jpg", ".png")
        save_path = os.path.join(SIM_IMAGE_DIR, image_name)

        if not os.path.exists(save_path):  # avoid re-download
            img_obj = s3.get_object(Bucket=MINIO_BUCKET, Key=image_key)
            image = Image.open(BytesIO(img_obj["Body"].read())).convert("RGB")
            image.save(save_path)

        # Path will map inside: /mnt/object/processed/processed_images/patient-sim/
        rel_path = f"CheXpert-v1.0/train/patient-sim/{image_name}"
        sim_records.append([rel_path] + label_vals)

    sim_df = pd.DataFrame(sim_records, columns=["Path"] + LABELS)
    train_df = pd.read_csv(TRAIN_CSV)
    train_df = pd.concat([train_df, sim_df], ignore_index=True)
    print(f"✅ Merged {len(sim_df)} simulation samples into training data.")
else:
    print("⚠️ No simulation prediction JSONs found. Using only original training set.")
    train_df = pd.read_csv(TRAIN_CSV)
...
# Log Metrics
# -----------------------
def log_metrics_to_mlflow(epoch, stage, metrics_dict):
...
class CheXpertDataset(Dataset):
    def __init__(self, data_source, root_dir, transform=None):
        if isinstance(data_source, str):
            self.df = pd.read_csv(data_source)
        else:
            self.df = data_source  # Already a DataFrame
    ...
    def __getitem__(self, idx):
        rel_path = self.image_paths[idx]  # e.g., patient123/... or patient-sim/...
...
# Load Data
# -----------------------
train_ds = CheXpertDataset(train_df, IMG_ROOT, transform)
...
with training_duration.time():
    with mlflow.start_run():
        ...
        mlflow.log_param("used_simulation_data", int('sim_df' in locals() and not sim_df.empty))
        ...
        for epoch in range(EPOCHS):
            ...
            log_metrics_to_mlflow(epoch, "train", {
                "loss": avg_train_loss,
            ...
            log_metrics_to_mlflow(epoch, "val", {
                "loss": avg_val_loss,
